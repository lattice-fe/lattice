...
/// Spawn the indexer worker on its own thread, owning the DB + embedding model.
/// Returns the command sender and the event receiver; the Tauri layer forwards
/// events to the frontend.
pub fn spawn() -> (std::sync::mpsc::Sender<Command>, tokio::sync::mpsc::Receiver<Event>) {
    let (cmd_tx, cmd_rx) = std::sync::mpsc::channel::<Command>();
    let (ev_tx, ev_rx) = tokio::sync::mpsc::channel::<Event>(512);
